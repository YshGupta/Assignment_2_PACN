...
tdma() {
    ...
    while [ "$cycle_count" -lt "$max_cycles" ]; do
        # echo 
        # Choosing a set randomly
        local random_index=$(( RANDOM % total_sets ))
        local random_set="${sets[random_index]}"

        # echo -e "\e[1;92m${random_set} activated\e[0m" | awk '{printf "%-70s %s\n", $0, ""}'
        execute_set "${!random_set}"  # randomly chosen set
        ((cycle_count++))
        # echo
    done
}
